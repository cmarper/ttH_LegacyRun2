...
void convert_tree(
  ...
  */
  ...
  for (int i=skip_entries;i<skip_entries+nentries;i++) {
	...
	if(daughter.Pt()>7 && fabs(daughter.Eta())<2.5 && fabs(dxy)<=0.05 && fabs(dz)<0.1 && miniRelIso_nanoAOD<0.4 && sip < 8 && LooseIdWP && eleMissingLostHits<=1){
          ...
          if(year==2016) leptonMVA = ele_reader_2016->EvaluateMVA("BDTG method");
	        ...
	        reco_eles.push_back(daughter_pair);
                //pair<int,TLorentzVector> daughter_pair_corr = make_pair(i_daughter,daughter_corr);
                //reco_eles_corr.push_back(daughter_pair_corr);
    ...
    //sort(reco_eles_corr.begin(), reco_eles_corr.end(), pT_comparison_pairs);
    ...
    for(unsigned int i_ele=0; i_ele<reco_eles.size(); i_ele++){
      ...
      float corr = (*_daughters_corr_ele)[i_daughter];
      float e_corr = (*_daughters_energy_corr)[i_daughter];
      float pt_corr = (*_daughters_pt_corr)[i_daughter];
      ...
      _recoele_corr_e.push_back( e_corr );
      _recoele_corr_pt.push_back( pt_corr );
    ...
    for(unsigned int i_ele=0; i_ele<reco_eles.size(); i_ele++){
      ...
      //TLorentzVector elec_corr = reco_eles_corr[i_ele].second;
