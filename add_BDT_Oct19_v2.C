...
void add_BDT(TString filein, TString fileout, vector<TString> treename){
  ...
  for(unsigned int i_tree=0;i_tree<treename.size();i_tree++){
    ...
    for(int i=0;i<nentries;i++){
      ...
      for ( int i_bjet = 0; i_bjet<_n_recoPFJet; i_bjet++) {
        ...
        for ( int i_Wjet1 = 0; i_Wjet1<_n_recoPFJet; i_Wjet1++) {
          ...
          for ( int i_Wjet2 = 0; i_Wjet2<_n_recoPFJet; i_Wjet2++) {
            ...
            _btagDisc_b = (*_recoPFJet_DeepJetscore)[i_bjet];
            _btagDisc_Wj1 = (*_recoPFJet_DeepJetscore)[i_Wjet1];
            _btagDisc_Wj2 = (*_recoPFJet_DeepJetscore)[i_Wjet2];
      ...
      //cout<<"("<<_BDTscore_res_HTT<<","<<_HadTop_pt<<"),("<<_BDTscore_res_HTT_2<<","<<_HadTop_pt_2<<")"<<endl;
      ...
      /*
      float _semiboosted_HTT_output = -1.;

      //cleaned jets from ak8 jets

      vector<TLorentzVector> cleaned_jets;
      vector<float> DeepJet_cleaned_jets;
      ...
      for ( int i_jet = 0; i_jet<_n_recoPFJet; i_jet++) {
        ...
        DeepJet_cleaned_jets.push_back((*_recoPFJet_DeepJetscore)[i_jet]);
      ...
      for ( int i_bjet = 0; i_bjet<n_cleaned_jets; i_bjet++) {
        ...
        for ( int i_ak8jet = 0; i_ak8jet<_n_recoPFak8Jet; i_ak8jet++) {
          ...
          _btagDisc_b = DeepJet_cleaned_jets.at(i_bjet);
          ...
          if ( bdtResult > _semiboosted_HTT_output ) {
            _semiboosted_HTT_output = bdtResult;
            _semiboosted_HTT = 1. / (1. + sqrt((1. - _semiboosted_HTT_output) / (1. + _semiboosted_HTT_output))); 
            _HadTop_pt = (bjet+ak8jet).Pt();
          }
        }
      }

      _BDTscore_semiboosted_HTT = _semiboosted_HTT;
      */
