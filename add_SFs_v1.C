...
void add_SFs(TString filein, TString fileout, vector<TString> treename, bool isMC){
  	...
  	*/
      ...
      tree_new->Branch("tauSF_weight_down_v1",&_tauSF_weight_down_v1,"tauSF_weight_down_v1/F");
        ...
        // New variables
        ...
        else if ( cat_2lss1tau || cat_2los1tau || cat_2l2tau ) {
      	...
      	if ( cat_1l1tau || cat_1l2tau ){

      			if((*_recolep_sel_ismvasel)[0])
        			_leptonSF_weight_v1 *= get_leptonSF(_year,(*_recolep_sel_pdg)[0],(*_recolep_sel_pt)[0],(*_recolep_sel_eta)[0],3); //no tight charge 
      			else
        			_leptonSF_weight_v1 *= get_RecoToLoose_leptonSF(_year,(*_recolep_sel_pdg)[0],(*_recolep_sel_pt)[0],(*_recolep_sel_eta)[0],3,0);

      	}

      	else if ( cat_2lss1tau ){

      		  if((*_recolep_sel_ismvasel)[0])
        			_leptonSF_weight_v1 *= get_leptonSF(_year,(*_recolep_sel_pdg)[0],(*_recolep_sel_pt)[0],(*_recolep_sel_eta)[0],2); //tight charge
      			else
      	...
      	else if ( cat_2l2tau || cat_2los1tau ){
        ...
        if ( cat_1l1tau || cat_2lss1tau || cat_2los1tau || cat_3l1tau){

          if( !(*_recotauh_sel_isGenMatched)[0] ) {

              TString wp;
              if((*_recotauh_sel_byMediumDeepTau2017v2p1VSjet)[0]==1) wp = "Medium";
              else if((*_recotauh_sel_byLooseDeepTau2017v2p1VSjet)[0]==1) wp = "Loose";
              else if((*_recotauh_sel_byVLooseDeepTau2017v2p1VSjet)[0]==1) wp = "VLoose";
              else wp = "VVLoose";
        ...
        else if ( cat_2tau || cat_1l2tau || cat_2l2tau){
          ...
          if( !(*_recotauh_sel_isGenMatched)[1] ) {
              ...
              else if((*_recotauh_sel_byLooseDeepTau2017v2p1VSjet)[1]==1) wp = "Loose";
              else if((*_recotauh_sel_byVLooseDeepTau2017v2p1VSjet)[1]==1) wp = "VLoose";
              else wp = "VVLoose";
...
void test(){
  	...
  	//filein.push_back("/data_CMS/cms/mperez/ttH_Legacy/Oct19/ntuples_splitted_BDT/2016/nominal/ttV/ntuple_Oct19v1_MC_2016_TTW_BDT.root");
  	...
  	//fileout.push_back("/data_CMS/cms/mperez/ttH_Legacy/Oct19/ntuples_splitted_BDT/2016/nominal/ttV/ntuple_Oct19v1_MC_2016_TTW_BDT_SFs.root");
