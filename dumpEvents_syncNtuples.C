...
void dump_events(TString region="2lSS_SR"){
	...
	in_tree_LLR->Add("/data_CMS/cms/mperez/ttH_Legacy/sync_ntuples/ntuples_syncformat/events/2016/sync_ntuple_events_syncformat_ttHNonbb_2016_v15.root");
	//in_tree_Tallinn->Add("/data_CMS/cms/mperez/ttH_Legacy/sync_ntuples/ntuples_groups/events/2016/sync_Tallinn_2016_v19.root");
	in_tree_IHEP->Add("/data_CMS/cms/mperez/ttH_Legacy/sync_ntuples/ntuples_groups/events/2016/sync_Tallinn_2016_v24.root");
...
void dump_objects(TString object="tau1_isGenMatched"){
	...
	in_tree_LLR->Add("/data_CMS/cms/mperez/ttH_Legacy/sync_ntuples/ntuples_syncformat/objects/2016/sync_ntuple_objects_syncformat_ttHNonbb_2016_v17.root");
	in_tree_Tallinn->Add("/data_CMS/cms/mperez/ttH_Legacy/sync_ntuples/ntuples_groups/objects/2016/sync_Tallinn_2016_v23.root");
	...
	Int_t _tau1_isGenMatched_LLR;
	Int_t _tau1_isGenMatched_Tallinn;

	in_tree_LLR->SetBranchAddress("nEvent",	&_nEvent_LLR);
	in_tree_Tallinn->SetBranchAddress("nEvent",	&_nEvent_Tallinn);

	in_tree_LLR->SetBranchAddress("tau1_isGenMatched", &_tau1_isGenMatched_LLR);
	in_tree_Tallinn->SetBranchAddress("tau1_isGenMatched", &_tau1_isGenMatched_Tallinn);
	...
	for (int i=0;i<nentries_LLR;i++) { 
		...
		if(entry_ok<0) 
			continue;
		if(_tau1_isGenMatched_LLR==0) continue;
		TString s_nEvent = Form("%.0i", _nEvent_LLR);
		out_file_LLR<<s_nEvent<<'\n';
	}
	...
	for (int i=0;i<nentries_Tallinn;i++) {
		...
		if(entry_ok<0) 
			continue;
		if(_tau1_isGenMatched_Tallinn==0) continue;
		TString s_nEvent = Form("%.0i", _nEvent_Tallinn);
		out_file_Tallinn<<s_nEvent<<'\n';
	}
