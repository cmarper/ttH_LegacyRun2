...
void plot_mu_comb(){
  ...
  double mu_2lss = +1.000;  
  double mu_2lss_m1s = 0.371;
  double mu_2lss_p1s = 0.421;
  cout<<"Mu 2lss = "<<mu_2lss<<" +"<<mu_2lss_p1s<<"/-"<<mu_2lss_m1s<<endl;

  double mu_3l = +1.000;  
  double mu_3l_m1s = 0.459;
  double mu_3l_p1s = 0.503;
  cout<<"Mu 3l = "<<mu_3l<<" +"<<mu_3l_p1s<<"/-"<<mu_3l_m1s<<endl;

  double mu_2lss1tau = +1.000;  
  double mu_2lss1tau_m1s = 0.438;
  double mu_2lss1tau_p1s = 0.528;
  cout<<"Mu 2lss1tau = "<<mu_2lss1tau<<" +"<<mu_2lss1tau_p1s<<"/-"<<mu_2lss1tau_m1s<<endl;

  double mu_4l = +1.000;  
  double mu_4l_m1s = 1.226;
  double mu_4l_p1s = 1.630;
  cout<<"Mu 4l = "<<mu_4l<<" +"<<mu_4l_p1s<<"/-"<<mu_4l_m1s<<endl;

  double mu_comb = +1.000;  
  double mu_comb_m1s = 0.272;
  double mu_comb_m1s_stat = 0.219;
  double mu_comb_p1s = 0.318;
  double mu_comb_p1s_stat = 0.224;
  ...
  gr_mu_comb_syst->GetXaxis()->SetLimits(-2.5,3.5);
  ...
  tex.SetTextSize(0.035);
  tex.DrawLatex(-4.25,y_2lss+0.3,"2lss + 0#tau_{h}");
  tex.DrawLatex(-4.25,y_2lss-0.4,Form("#bf{#mu = %.2f ^{+%.2f}_{-%.2f}}",mu_2lss,mu_2lss_p1s,mu_2lss_m1s));
  tex.DrawLatex(-4.25,y_3l+0.3,"3l + 0#tau_{h}");
  tex.DrawLatex(-4.25,y_3l-0.4,Form("#bf{#mu = %.2f ^{+%.2f}_{-%.2f}}",mu_3l,mu_3l_p1s,mu_3l_m1s));
  tex.DrawLatex(-4.25,y_2lss1tau+0.3,"2lss + 1#tau_{h}");
  tex.DrawLatex(-4.25,y_2lss1tau-0.4,Form("#bf{#mu = %.2f ^{+%.2f}_{-%.2f}}",mu_2lss1tau,mu_2lss1tau_p1s,mu_2lss1tau_m1s));
  tex.DrawLatex(-4.25,y_4l+0.3,"4l + 0#tau_{h}");
  tex.DrawLatex(-4.25,y_4l-0.4,Form("#bf{#mu = %.2f ^{+%.2f}_{-%.2f}}",mu_4l,mu_4l_p1s,mu_4l_m1s));
  ...
  tex.SetTextSize(0.035);
  tex.DrawLatex(-2.0,10,Form("#mu = %.3f ^{+%.3f}_{-%.3f}  #bf{#left[ {}^{+%.3f}_{-%.3f}(stat.) {}^{+%.3f}_{-%.3f}(syst.)#right]}",mu_comb,mu_comb_p1s,mu_comb_m1s,mu_comb_p1s_stat,mu_comb_m1s_stat,mu_comb_p1s_syst,mu_comb_m1s_syst));
  
  c->SaveAs("mu_ttH_exp.pdf");
  c->SaveAs("mu_ttH_exp.png");
