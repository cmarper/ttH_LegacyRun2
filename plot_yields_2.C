...
void plot_categories(){
  ...
  TString category[10] = {"0l+2#tau_{h}", "1l+1#tau_{h}", "1l+2#tau_{h}", "2los+1#tau_{h}", "2l+2#tau_{h}", "3l+1#tau_{h}", "4l+0#tau_{h}", "3l+0#tau_{h}", "2lss+0#tau_{h}", "2lss+1#tau_{h}"};

  float ttH[10] = {27.7, 192.9, 21.8, 21.2, 2.4,  4.6, 2.0, 60.5, 219.4, 33.6};
  float tH[10] = {2.9, 10.9, 0.5, 0.8, 0,  0.2, 0, 3.3, 19.9, 2.4};
  float ttZ[10] = {26.9, 187.2,20.3, 26.5,2.5, 6.7,5.6, 131.3, 292.2, 31.4};
  float ttW[10] = { 2.7, 177.7, 2.0, 13.0,0, 1.0,0.1, 113.0, 766.4, 38.0};
  float WZ[10] = {30.2, 131.1, 8.4, 5.6,0,0.4, 0, 63.0, 219.0, 15.1};
  float ZZ[10] = {25.2, 64.9,1.2,1.2,0.1,0.2,0.3, 10.2, 18.6,1.1};
  float DY[10] = {1112.1, 3539.4, 0, 0, 0, 0, 0, 0, 0, 0};
  float ttbar[10] = {888.1, 39811.7, 0, 0, 0, 0, 0, 0, 0, 0};
  float Fakes[10] = {3637.3, 23602.0, 241.6,410.3,3.9, 1.3, 0, 175.1, 1539.2, 54.7};
  float Flips[10] = {0, 0, 0, 0, 0, 0, 0, 0, 139.5,1.0};
  float Convs[10] = {0, 0, 0, 0.5, 0, 0, 0, 5.3, 39, 4};
  float Rares[10] = {57.7, 1738.9,5.5,5.6,0.3,1.0,0.6, 37.7, 197.7, 13.4};
  float Higgs[10] = {24.2, 34.0, 0, 0.8,0, 0, 0,3.3, 29.8,1.8};
  float Total[10] = {5807.4, 69297.8, 279.5,464.0,6.9,  10.7,6.7, 537.0, 3222.3, 158.8};
  ...
  TH1F *h_Convs = new TH1F("h_Convs","h_Convs",10,0,10);
  for (int i=0; i<10; i++){
    h_Convs->AddBinContent(i+1,Convs[i]);
    h_Convs->SetBinError(i+1,0.000001);
    h_Convs->GetXaxis()->SetBinLabel(i+1,category[i]);
  }
  ...
  TLatex *texl1 = new TLatex(0.1,250000,"#bf{CMS} #it{Preliminary}");
  texl1->SetTextSize(0.05);
  texl1->SetTextFont(42);

  TLatex *texl2 = new TLatex(6.9,250000,"137 fb^{-1} (13 TeV)");
  ...
  TLegend* leg2=new TLegend(0.45,0.59,0.60,0.88);
  ...
  leg2->AddEntry(h_Convs,"Conversions", "f");
